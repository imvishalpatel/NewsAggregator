...
<div class="row panel">
			...
			<article class="col-md-8 maincontent" style="margin-top: 44px;">
                                     <%
                                         ...
                                         //out.println("SIZE IS "+a);
                                         PrivateDiscussion pb = null;
                                         ArrayList<String> ab = null;
            try{
                     ArrayList<String> tag=new ArrayList<String>();
            
              pb = (PrivateDiscussion)request.getAttribute("pb");
              ab = new ArrayList<String>();
              ab = pb.getUserlist();
              
//              for(int i=0;i<ab.size();i++)
//                  out.println("<h1>"+ab.get(i)+"<br><br><h1>");
						...
						<table border="0" id="Comment" width="800">
								...
								<td>
                                                                    <%out.println(comm.get(j).getComment());
                                                                        ...
                                                                        <br/></br>
<!--                                                                        comment vote-->
<a class="<%if(comm.get(j).getVoteValue(username)==1)out.print("fa fa-thumbs-up upvote");else out.print("fa fa-thumbs-up");%>" aria-hidden="true" style="text-decoration:none; cursor:pointer" onclick="upvoteComment('<%=username%>' , '<%= pb.getid() %>', '<%=comm.get(j).getId()%>', 'upvote<%=j%>', 'downvote<%=j%>' )"> </a> <label id='upvote<%=j%>'><%out.println(comm.get(j).getUpVotes());%></label>&nbsp;
                                                                        <a class="<%if(comm.get(j).getVoteValue(username)==-1)out.print("fa fa-thumbs-down downvote");else out.print("fa fa-thumbs-down");%>" aria-hidden="true" style="text-decoration:none; cursor :pointer" onclick="downvoteComment('<%=username%>' , '<%= pb.getid() %>', '<%=comm.get(j).getId()%>', 'upvote<%=j%>', 'downvote<%=j%>' )"></a><label id='downvote<%=j%>'><%=comm.get(j).getDownVotes()%></label>&nbsp;
...
<!--                                                                        <a href="#" id="cmt_on_cmt">add a comment</a>-->
			...
			<aside class="col-md-2 sidebar sidebar-right" style="padding-left: 15px; margin-left: 15px; width: 235px; padding-top: 26px;">
				<form name="private-post-invitee">
					<h3 style="color:#3366FF;">Invited Users : </h3>
					<table border="0" cellpadding="6" id="invitee-table">
                                            <%
                                                    String list ="";
                                                if(ab!=null){
                                                   for(int i=0;i<ab.size();i++){
                                                       list="";
                                                        list+="<tr><td><a href='#'>"+ab.get(i)+"</a></td>";
							list+="<td><button type='submit' id='mut-1' value='Mute'>Mute</button></td>";
							list+="<td><button type='submit' id='rmv-1' value='Remove'>Remove</button></td></tr>";
                                                        out.println(list);
                                                   }
                                               }
                                                else
...
//                                                
                                                %>	
                                            <tr>
							<td><a href="#">pmraut_29</a></td>
							<td><button type='submit' id="mut-1" value='Mute'>Mute</button></td>
							<td><button type='submit' id="rmv-1" value='Remove'>Remove</button></td>
						</tr>
